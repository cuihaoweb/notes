...
const miniSearchIndex = computedAsync(async () => {
    const searchData = (await searchIndexData.value[localeIndex.value]?.())?.default;
...
debouncedWatch([miniSearchIndex, filterText], async ([miniSearch, filterTextValue]) => {
